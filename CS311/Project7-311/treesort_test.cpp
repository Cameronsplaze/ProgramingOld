...
// Requires catch.hpp, treesort.h
//
// History:
// - v2: Improved stability tests
...
public:
    ...
    /* DELETION OF MOVE OPS COMMENTED OUT IN THIS TEST PROGRAM
    // No move operations
           ...
           "[other]" )
{
    SECTION( "Count additional value-type objects used in sort" )
    {
        ...
        std::vector<Counter> vc(DATASIZE);
        size_t base_existing = Counter::getExisting();
        treesort(vc.begin(), vc.end());
        size_t add_objs = Counter::getMaxExisting() - base_existing;
    ...
    SECTION( "Check for proper clean-up after sort" )
    {
        ...
        REQUIRE( Counter::getExisting() == base_existing );
        }
    }
}
           ...
           "[actions]" )
{
    SECTION( "Larger list" )
    {
        const size_t DATASIZE = size_t(100000);
        vector<int> data;
